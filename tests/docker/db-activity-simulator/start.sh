...
while /bin/true ; do
    psql ${DSN_ACTIVITY_SIMULATOR} -c "insert into sample_table_activity (name) values (gen_random_uuid())"
    ...
    psql ${DSN_ACTIVITY_SIMULATOR} -c "update sample_table_activity set name = 'updated ' || gen_random_uuid() where id = (select min(id) from sample_table_activity);"
    sleep 1
done
